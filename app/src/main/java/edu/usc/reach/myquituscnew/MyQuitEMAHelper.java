    ...
    public static void setUpEODEMA(boolean isMainStudy) {
        if(isMainStudy) {
            ...
            if (nowTime.get(Calendar.HOUR_OF_DAY) >= KEY_EOD_PROMPT_TIME()[0] && nowTime.get(Calendar.MINUTE) >= KEY_EOD_PROMPT_TIME()[1] &&
                    MyQuitCSVHelper.isLastEventPastXMinutesTrue(MyQuitCSVHelper.END_OF_DAY_EMA_KEY, (60 * (24 - (KEY_EOD_PROMPT_HOUR_CUT()/60))))) {
            ...
            Calendar nowTime = Calendar.getInstance();
            if (nowTime.get(Calendar.HOUR_OF_DAY) >= KEY_EOD_PROMPT_TIME()[0] && nowTime.get(Calendar.MINUTE) >= KEY_EOD_PROMPT_TIME()[1] &&
                    MyQuitCSVHelper.isLastEventPastXMinutesTrue(MyQuitCSVHelper.PQ_END_OF_DAY_EMA_KEY, (60 * (24 - (KEY_EOD_PROMPT_HOUR_CUT()/60))))) {
